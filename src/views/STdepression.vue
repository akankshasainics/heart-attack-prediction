<template>
    <div id="st">
        ...
        <h2> ST depression induced by exercise relative to rest </h2>
         <fieldset class="material">
          <input v-on:keyup.enter="setSTdepression($event.target.value)" v-model.lazy ="selected" type="float" placeholder="eg. 1">
        </fieldset>
        </div>
        <div class="error valid" v-if="!$v.selected.between">Must be a number.</div>
        <div class="error" v-if="$v.selected.between && $store.state.arr[9] != -1">
            <h2> Entered: {{arr[9]}} </h2>
        </div>
    </div>
</template>
